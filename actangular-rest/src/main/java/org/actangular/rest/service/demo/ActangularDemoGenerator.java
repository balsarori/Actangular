...
public class ActangularDemoGenerator extends DemoDataGenerator {
  ...
  protected int demoTasksPerUser = 0;
  ...
  protected void initDemoUsers() {
    createUser("kermit", "Kermit", "The Frog", "kermit", "kermit@activiti.org", "org/activiti/rest/images/kermit.jpg", Arrays.asList(
  ...
  protected void createDemoTasks() {
    ...
    for (User user : users) {
      ...
      for (int i = 0; i < demoTasksPerUser; i++) {
        ...
        if(i%2 ==0)
          assignee = users.get(randomGenerator.nextInt(userCount)).getId();
        
        Task ownedTask = createTask(user.getId() + " owned Task " + (i + 1), user.getId(), assignee,
            priorities[randomGenerator.nextInt(priorities.length)], cats[randomGenerator.nextInt(cats.length)]);
        
        taskService.addUserIdentityLink(ownedTask.getId(), users.get(randomGenerator.nextInt(userCount)).getId(),
